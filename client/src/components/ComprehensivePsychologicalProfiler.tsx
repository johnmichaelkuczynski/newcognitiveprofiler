...
import { ChevronDown, ChevronUp, MessageCircle, Send, User, Bot, Heart, Upload, Loader2 } from 'lucide-react';
import { useComprehensivePsychologicalAnalysis } from '@/hooks/useComprehensivePsychologicalAnalysis';
import { MultiProviderComprehensivePsychologicalResult, ComprehensivePsychologicalParameter, ModelProvider, DialogueEntry } from '@/types/analysis';
...
export default function ComprehensivePsychologicalProfiler({ initialText = '', onNewAnalysis }: ComprehensivePsychologicalProfilerProps) {
  ...
  const {
    ...
    data,
    reset
  } = useComprehensivePsychologicalAnalysis();
  ...
  const handleDialogueSubmit = async () => {
    ...
    const aiEntry: DialogueEntry = {
      ...
      timestamp: new Date().toISOString(),
      type: 'ai',
      message: `I understand your perspective on "${dialogueInput}". Let me consider this additional context and provide more psychological insights based on the comprehensive analysis.`,
    };
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-lg">Analyzing comprehensive psychological patterns...</p>
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Analyze text across 20 specific psychological parameters with detailed quotations and reasoning
            </CardDescription>
          </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span>Comprehensive Psychological Profile</span>
            <Button onClick={handleReset} variant="outline" size="sm">
              New Analysis
            </Button>
          </CardTitle>
          <CardDescription>
            Analysis of {data.originalText.length} characters across 20 psychological parameters
          </CardDescription>
        </CardHeader>
        <CardContent>
...
interface ParameterCardProps {
  parameter: ComprehensivePsychologicalParameter;
  isExpanded: boolean;
  onToggle: () => void;
}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {parameter.quotations.map((quote, index) => (
                    <blockquote key={index} className="border-l-4 border-purple-500 pl-4 italic text-sm">
                      "{quote}"
                    </blockquote>
                  ))}
                </div>
